...
class Graph:
        ...
        for k in self.streets.keys():
            ...
            for i in range(len(cors)-1):
                if len(cors[i])>2 and len(cors[i+1])>2:
                    ...
                    b=Point(cors[i+1])
                    c=Point(cors[i])
                    is_on = is_between(a, c, b)
                    if not is_on:                        
                        #tmp = self.streets[k][i]
                        #self.streets[k][i]=cors[i+1]
                        #self.streets[k][i+1] = tmp
                        tmp = cors[i]
                        cors[i] = cors[i+1]
                        cors[i+1] = tmp
    ...
    def add_node(self, node_id, value):
        v = Node(node_id, value)  # instantiate Node object
    ...
    def print_all_edges_one_line(self):#print all edges for current graph
        outputs  = []
        for k in self.streets:
            outputs.extend(self.get_street_edges(k))
        line_output = "E {"
        if len(outputs)> 0:
            line_output += ",".join(outputs)
        line_output += '}'
        print(line_output, flush=True)
    #def print_all_edges(self):#print all edges for current graph
        #outputs  = []
        #for k in self.streets:
            #outputs.extend(self.get_street_edges(k))
        #if len(outputs)> 0:
            #print(",\n".join(outputs), flush=True)
    def get_street_edges(self, street_name): #print all the edges of node with id node_id
        outputs = []
...
def gg_graph(streets):
    ...
    g.generate_graph()
    #g.print_graph()
    print("V {}".format(len(g.nodes)), flush=True)
    g.print_all_edges_one_line()
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if not line:
            break
        # split the line into words using a whitespace
        command = ""
        index = line.find(" ")
        command = line[:index]
        if index == -1:
